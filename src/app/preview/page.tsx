...
export default function PreviewPage() {
  return (
    <main className="min-h-[100dvh] bg-[var(--card-muted)]">
      <header className="border-b border-[var(--border)] bg-[var(--ink)] text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="flex size-9 items-center justify-center rounded-md bg-[var(--accent)] font-mono font-bold">52</div>
            <div>
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-6 py-8 lg:grid-cols-[1fr_360px]">
        <div className="space-y-6">
          <div className="flex flex-col justify-between gap-4 rounded-lg border border-[var(--border)] bg-[var(--card)] p-6 md:flex-row md:items-end">
            <div>
            ...
            <CardContent>
              <div className="grid gap-3 md:grid-cols-3">
                {route.map((location, index) => (
                  <div key={location} className="rounded-md border border-[var(--border)] bg-[var(--card-muted)] p-3">
                    <p className="font-mono text-sm text-[var(--accent)]">#{index + 1}</p>
                    <p className="mt-1 font-medium">{location}</p>
                    <p className="mt-1 text-sm text-[var(--muted)]">Bright, exaggerated, and easy to revisit.</p>
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Images className="size-5 text-[var(--accent)]" />
                Study mode
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                {studyCards.map((card, index) => (
                  <div key={card.label} className="rounded-lg border border-[var(--border)] bg-[var(--card-muted)] p-4">
                    ...
                    <p className="mt-1 text-sm leading-6 text-[var(--muted)]">{card.image}</p>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <aside className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {mistakes.map((mistake) => (
                <div key={mistake} className="rounded-md bg-[var(--card-muted)] p-3 text-sm leading-6 text-[var(--foreground)]">
                  {mistake}
                </div>
              ))}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {["Q♥", "5♣", "9♦"].map((card) => (
                <div key={card} className="flex items-center justify-between rounded-md bg-[var(--accent-soft)] px-3 py-2">
                  <span className="font-mono font-semibold text-[var(--accent)]">{card}</span>
                  <span className="text-sm text-[var(--muted)]">Due now</span>
                </div>
              ))}
            </CardContent>
          </Card>
